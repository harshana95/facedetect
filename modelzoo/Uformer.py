...
def get_arch(opt):
    arch = opt.arch
    train_ps = opt.image_size_h if opt.patch_size <= 0 else opt.patch_size
    embed_dim = 32
    dd_in = 3
    c = opt.n_colors
    ...
    if arch == 'Uformer':
        m = Uformer(img_size=train_ps,embed_dim=embed_dim,win_size=8,token_projection='linear',token_mlp='leff',modulator=True, in_chans=c)
    elif arch == 'Uformer_T':
        m = Uformer(img_size=train_ps,embed_dim=16,win_size=8,token_projection='linear',token_mlp='leff',modulator=True, in_chans=c)
    elif arch == 'Uformer_S':
        m = Uformer(img_size=train_ps,embed_dim=32,win_size=8,token_projection='linear',token_mlp='leff',modulator=True, in_chans=c)
    elif arch == 'Uformer_S_noshift':
        m = Uformer(img_size=train_ps,embed_dim=32,win_size=8,token_projection='linear',token_mlp='leff',modulator=True,
            shift_flag=False, in_chans=c)
    elif arch == 'Uformer_B_fastleff':
        m = Uformer(img_size=train_ps,embed_dim=32,win_size=8,token_projection='linear',token_mlp='fastleff',
            depths=[1, 2, 8, 8, 2, 8, 8, 2, 1],modulator=True, in_chans=c)
    elif arch == 'Uformer_B':
        m = Uformer(img_size=train_ps,embed_dim=32,win_size=8,token_projection='linear',token_mlp='leff',
            depths=[1, 2, 8, 8, 2, 8, 8, 2, 1],modulator=True,dd_in=dd_in, in_chans=c)
    else:
...
class FastLeFF(nn.Module):
    ...
    def __init__(self, dim=32, hidden_dim=128, act_layer=nn.GELU, drop=0.):
        ...
        self.dwconv = nn.Sequential(Conv2d(hidden_dim, hidden_dim, kernel_size=3, stride=1, padding=1, groups=hidden_dim), act_layer())
